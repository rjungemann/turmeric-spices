#!/usr/bin/env bash
# errors/run.sh -- assert the c-dsl compile-fail diagnostics.
#
...
echo "# the c-dsl compile-fail diagnostics"

expect_reject carr-length-mismatch.tur TUR-E0260 \
  'function '\''c-arr-copy-stmt'\'' shares size variable '\''n'\'' across parameters, but argument 1 has size 16 while argument 2 has size 8' \
  'c-arr-copy-stmt rejects a copy between arrays of different lengths'
