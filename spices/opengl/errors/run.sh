#!/usr/bin/env bash
# errors/run.sh -- assert the opengl compile-fail diagnostics (substructural handle discipline).
#
...
echo "# the opengl compile-fail diagnostics (substructural handle discipline)"

expect_reject opengl-shader-use-after-link.tur TUR-E0101 \
  'linear value '\''vs'\'' used after being consumed' \
  'deleting a shader already consumed by the link step is rejected'
expect_reject opengl-vbo-leak.tur TUR-E0100 \
  'linear value '\''vbo'\'' dropped without being consumed' \
  'a VBO that is never deleted is a leak'
expect_reject opengl-vbo-use-after-delete.tur TUR-E0101 \
  'linear value '\''vbo'\'' used after being consumed' \
  'binding a deleted VBO is a use-after-consume'
expect_reject opengl-window-leak.tur TUR-E0100 \
  'linear value '\''w'\'' dropped without being consumed' \
  'a window that is never destroyed is a leak'
expect_reject opengl-window-use-after-destroy.tur TUR-E0101 \
  'linear value '\''w'\'' used after being consumed' \
  'swapping buffers on a destroyed window is a use-after-consume'
