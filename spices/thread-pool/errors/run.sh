#!/usr/bin/env bash
# errors/run.sh -- assert the thread-pool compile-fail diagnostics.
#
...
echo "# the thread-pool compile-fail diagnostics"

expect_reject forgotten-stop.tur TUR-E0100 \
  'linear value '\''p'\'' dropped without being consumed' \
  'a pool that is never stopped is a leak'
expect_reject double-stop.tur TUR-E0101 \
