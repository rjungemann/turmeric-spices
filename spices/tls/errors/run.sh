#!/usr/bin/env bash
# errors/run.sh -- assert the tls compile-fail diagnostics.
#
...
echo "# the tls compile-fail diagnostics"

expect_reject tls-double-free.tur TUR-E0101 \
  'linear value '\''conn'\'' used after being consumed' \
  'freeing a TLS connection twice is a use-after-consume'
expect_reject tls-leak-no-free.tur TUR-E0100 \
  'linear value '\''conn'\'' dropped without being consumed' \
  'a TLS connection that is never freed is a leak'
expect_reject tls-use-after-free.tur TUR-E0101 \
  'linear value '\''conn'\'' used after being consumed' \
  'reading from a freed TLS connection is a use-after-consume'
