#!/usr/bin/env bash
# errors/run.sh -- assert the linalg compile-fail diagnostics.
#
...
echo "# the linalg compile-fail diagnostics"

expect_reject sized-dim-mismatch.tur TUR-E0260 \
  'function '\''lavecn-dot'\'' shares size variable '\''n'\'' across parameters, but argument 1 has size 2 while argument 2 has size 3' \
  'lavecn-dot rejects vectors of different lengths'
expect_reject sized-matrix-dim-mismatch.tur TUR-E0260 \
  'function '\''lamat-mul'\'' shares size variable '\''k'\'' across parameters, but argument 1 has size 3 while argument 2 has size 5' \
  'lamat-mul rejects a mismatched inner dimension'
expect_reject sized-matrix-dim-mismatch.tur TUR-E0260 \
  'function '\''lamat-add'\'' shares size variable '\''n'\'' across parameters, but argument 1 has size 3 while argument 2 has size 4' \
  'lamat-add rejects operands of different shapes'
