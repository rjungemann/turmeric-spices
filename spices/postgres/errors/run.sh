#!/usr/bin/env bash
# errors/run.sh -- assert the postgres compile-fail diagnostics.
#
...
echo "# the postgres compile-fail diagnostics"

expect_reject postgres-conn-leak.tur TUR-E0100 \
  'linear value '\''conn'\'' dropped without being consumed' \
  'a connection that is never closed is a leak'
expect_reject postgres-conn-use-after-close.tur TUR-E0101 \
  'linear value '\''conn'\'' used after being consumed' \
  'querying a closed connection is a use-after-consume'
expect_reject postgres-rows-leak.tur TUR-E0100 \
  'linear value '\''rows'\'' dropped without being consumed' \
  'a result set that is never freed is a leak'
expect_reject postgres-rows-use-after-free.tur TUR-E0101 \
