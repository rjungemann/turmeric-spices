#!/usr/bin/env bash
# errors/run.sh -- assert the sqlite compile-fail diagnostics.
#
...
echo "# the sqlite compile-fail diagnostics"

expect_reject sqlite-db-double-close.tur TUR-E0101 \
  'linear value '\''db'\'' used after being consumed' \
  'closing a database twice is a use-after-consume'
expect_reject sqlite-db-use-after-close.tur TUR-E0101 \
  'linear value '\''db'\'' used after being consumed' \
  'querying a closed database is a use-after-consume'
expect_reject sqlite-stmt-leak.tur TUR-E0100 \
  'linear value '\''st'\'' dropped without being consumed' \
  'a statement that is never finalized is a leak'
expect_reject sqlite-stmt-use-after-finalize.tur TUR-E0101 \
  'linear value '\''st'\'' used after being consumed' \
  'stepping a finalized statement is a use-after-consume'
