---
description: Cut a new major release of a single spice. Bump build.tur :version, update CHANGELOG, deploy docs, push tag.
argument-hint: <spice-name>
allowed-tools: Bash, Read, Edit, AskUserQuestion
---

# Cut a new major release of a spice

Run a full major-version release of one spice in this workspace.
...
A major bump signals **breaking changes** to this spice's public API
(exports, function signatures, behavior contracts). Be extra deliberate
about the changelog: the **Breaking changes** section is the most
important part of a major release and should lead the entry, with
explicit migration notes for downstream consumers.
...
Compute `NEW = (MAJOR+1).0.0`.

Example: `0.1.4` -> `1.0.0`.

Because this is a major bump, confirm with the user that the breaking
changes since `<name>-v<OLD>` actually justify a major-version bump (vs.
a minor) before going further. If they do not, refuse to proceed and
suggest `/cut-minor-release <name>` instead.
...
- **Breaking changes** -- renames, signature changes, removed exports,
  semantic shifts that require downstream user code edits. **Lead the
  entry with this section** for a major release.
...
- **Removed** -- deletions of features, modules, or exports (also note in Breaking)
...
migration note (see `spices/plot/CHANGELOG.md` 0.2.0 and
`spices/frame/CHANGELOG.md` 0.2.0 for the in-repo style). Downstream
users will be reading this to update their code.
...
(they use `## <VERSION>` headers, not `## [<VERSION>]`):
...
Omit empty subsections. Lead each bullet with a short bolded title
where appropriate, matching the existing entries' style.

If `spices/<name>/CHANGELOG.md` does **not** exist, create it with this
template (the **Breaking changes** section is mandatory for a major
bump even on a fresh CHANGELOG):
...
Show the user:
- The spice name and the OLD -> NEW version transition (and a reminder
...
emphasizing the breaking changes>
...
Report to the user:
- The spice name and new version (and that this is a major release)
...
- Refuse to skip the CHANGELOG update -- per-spice CHANGELOGs are the
  primary release-notes surface for spice consumers, and the
  **Breaking changes** section is especially important for a major bump.
...
- Refuse to cut a major release when there are no actual breaking
  changes since the previous tag -- suggest `/cut-minor-release <name>`
  instead.
